...
def plot(
    ...
    if plot_circle:
        ...
        if corrected_signal is None:
            ...
            ax.scatter(np.real(signal), np.imag(signal), s=size, facecolors=facecolors, edgecolors='C0', alpha=alpha_fit)
            if fit is not None:
                ax.plot(np.real(fit), np.imag(fit), "-C1", zorder=zorder)
        else:
            if plot_not_corrected:
                # ax.plot(np.real(signal), np.imag(signal), ".C0", alpha=0.15)
                ax.scatter(np.real(signal), np.imag(signal), s=size, facecolors=facecolors, edgecolors='C0', alpha=0.15*alpha_fit)
            # ax.plot(np.real(corrected_signal), np.imag(corrected_signal), ".C0")
            ax.scatter(np.real(corrected_signal), np.imag(corrected_signal), s=size, facecolors=facecolors, edgecolors='C0', alpha=alpha_fit)
            if fit is not None:
                if plot_not_corrected:
                    ax.plot(np.real(fit), np.imag(fit), "-C1", alpha=0.15, zorder=zorder)
                ax.plot(np.real(corrected_fit), np.imag(corrected_fit), "-C1", zorder=zorder)
    ...
    ax.scatter(freq_disp, dB(signal), s=size, facecolors=facecolors, edgecolors='C0', alpha=alpha_fit)
    if fit is not None:
    ...
    if fit_params_label is not None:
        ax.legend()
    ax.grid(alpha=0.3)
    ax.set_ylabel(r"$|%s|$ [dB]"%y_axis_str)
    ...
    if corrected_signal is None:
        ...
        ax.scatter(freq_disp, deg(signal), s=size, facecolors=facecolors, edgecolors='C0', alpha=alpha_fit, label=params_label)
        if fit is not None:
            ax.plot(freq_disp, deg(fit), "-C1", zorder=zorder)
    else:
        if plot_not_corrected:
            # ax.plot(freq_disp, deg(signal), ".C0", alpha=0.15)
            ax.scatter(freq_disp, deg(signal), s=size, facecolors=facecolors, edgecolors='C0', alpha=0.15*alpha_fit)
        # ax.plot(freq_disp, deg(corrected_signal), ".C0", label=params_label)
        ax.scatter(freq_disp, deg(corrected_signal), s=size, facecolors=facecolors, edgecolors='C0', alpha=alpha_fit, label=params_label)
        if fit is not None:
            if plot_not_corrected:
        ...
        ax.set_ylim(
            angle_center - 1.1 * angle_span, angle_center + 1.1 * angle_span,
        )

    if params_label is not None:
        ax.legend()
    ax.grid(alpha=0.3)
    ax.set_ylabel(r"$\arg(%s)$ [deg]"%y_axis_str)
    ax.set_xlabel("f [%sHz]" % freq_prefix)
